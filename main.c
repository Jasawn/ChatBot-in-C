...
int main(int argc, char *argv[]) {
	...
	do {
		...
		do {
			...
			/* split it into words */
			inc = 0;
			inv[inc] = strtok_s(input, delimiters, &idk);
			while (inv[inc] != NULL) 
			{
				...
				/* go to the next word */
				inc++;
				inv[inc] = strtok_s(NULL, delimiters, &idk);
			}
		} while (inc < 1);
		/* invoke the chatbot */
		done = chatbot_main(inc, inv, output, MAX_RESPONSE); //parse all the value to chatbot_main
...
{
	int i = 0;
	while (token1[i] != '\0' && token2[i] != '\0') //Keep looping until the end of string
	{
		if (toupper(token1[i]) < toupper(token2[i]))
			return -1; //Token 1 is smaller than Token 2
		else if (toupper(token1[i]) > toupper(token2[i]))
			return 1; //Token 1 is bigger than Token 2
		i++;
	}

	if (token1[i] == '\0' && token2[i] == '\0') //Both is equal
		return 0; 
	else if (token1[i] == '\0') //Token 2 string is longer than Token 1
		return -1;
	else
		return 1; //Token 1 string i longer than Token 2
...
void prompt_user(char *buf, int n, const char *format, ...) 
{
	...
	fgets(buf, n, stdin); //Get the input from user and stores as buf
	char *nl = strchr(buf, '\n'); //look for \n
	if (nl != NULL) //Concat the string after the \n
	{
		*nl = '\0';
	}
}
